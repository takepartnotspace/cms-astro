---
seoTitle: Take Part not space
blocks:
  - headline: 'Männer, wir müssen reden. Und handeln.'
    tagline: 'Gegen patriarchale Gewalt hilft kein Wegschauen. Schluss mit Schulterzucken und Schweigen. Ob ein blöder Spruch am Stammtisch, das Übergehen von Grenzen im Bekanntenkreis oder Gewalt in ihrer schwersten Form: Männergewalt ist Alltag. Weil wir als Männer Teil des strukturellen Problems sind, müssen wir auch Teil der Lösung sein. Es ist nicht die Aufgabe der betroffenen Personen, uns aufzuklären oder den Kampf gegen Gewaltstrukturen allein zu führen. Wir machen unsere Hausaufgaben selbst, schauen im eigenen Umfeld hin und sorgen für ein aufrichtiges Miteinander.'
    actions:
      - label: Kontaktiere uns
        type: button
        icon: mail
        link: 'mailto:hello@takepartnotspace.org'
    image:
      src: ''
    starfield: true
    _template: hero
  - title: Wer wir sind
    body: |
      Take Part not space ist eine Initiative vielseitiger Männer aus Kultur, Sport, Wissenschaft und Gesellschaft. Wir organisieren uns, um patriachale Muster kritisch zu hinterfragen, feministische Forderungen aktiv zu unterstützen und andere Männer für eine gerechte und gewaltfreie Gesellschaft zu gewinnen.
    image:
      src: /watermarked_img_10756012750985512358.jpg
    reverse: true
    actions:
      - label: Mitwirkende und Unterstützer
        type: button
        icon: circle-arrow-right
        link: /mitwirkende
    _template: split
  - title: Unsere Motivation
    body: |
      Geschlechtsspezifische Gewalt, Abwertung und Hass gegen Frauen sowie queeren Menschen nehmen seit Jahren spürbar zu. Gleichzeitig heizt eine antifeministische Männerbewegung (Manosphere) diese Entwicklungen noch weiter an: Anstatt Probleme zu lösen, nutzen bestimmte Gruppen solche Ideologien gezielt aus, um Menschen gegeneinander auszuspielen.
    image:
      src: /jack-sharp-ShCVvQbQBDk-unsplash.jpg
    actions:
      - label: Orte & Aktionen
        type: button
        icon: circle-arrow-right
        link: /blog
    _template: split
  - title: Gleichberechtigung ist Männersache
    body: |
      Seit Jahrzehnten kämpfen betroffene Personen überwiegend allein gegen patriarchale Strukturen und Gewalt. Es darf nicht ihre Aufgabe sein, das Problem zu lösen, das wir Männer verursachen. Ein sicheres Miteinander entsteht erst dann, wenn wir nicht mehr nur zusehen, sondern selbst aktiv werden und Verantwortung übernehmen.
    image:
      src: /Screenshot-2026-08-02-190102.png
    reverse: true
    actions:
      - label: Fragen & Antworten
        type: button
        icon: circle-arrow-right
        link: /
    _template: split
  - title: Unsere Überzeugungen
    description: 'Vier Prinzipien, die unser Handeln und unsere Initiativen leiten.'
    items:
      - icon: friends
        title: Selbstreflexion statt Selbstgerechtigkeit
        text: |
          Wir hinterfragen eigene Verhaltensmuster, Privilegien und Rollenbilder kritisch.
      - icon: hand-stop
        title: Verantwortung im Alltag übernehmen
        text: |
          Wir schauen hin, wenn Grenzen überschritten werden, und schreiten bei Sprüchen oder Übergriffen ein.
      - icon: message-circle
        title: Austausch und gegenseitiger Rückhalt
        text: |
          Überlastung und Druck führen oft zu Aggression. Wir ermutigen zu ehrlichen Gesprächen über mentale Gesundheit.
      - icon: mood-happy
        title: Solidarität ohne Vereinnahmung
        text: |
          Wir unterstützen feministische Bündnisse solidarisch, ohne Bedingungen zu stellen oder Sorgearbeit abzuladen.
    _template: features
  - title: Unterstützung und Hilfe bei Gewalt
    description: 'Verantwortung übernehmen heißt auch, sich rechtzeitig Hilfe zu holen. Egal ob du selbst von Gewalt betroffen bist oder merkst, dass du in deinem Umfeld zu Aggression, Kontrolle oder Übergriffen neigst: Es gibt kostenlose und vertrauliche Anlaufstellen für Gewaltprävention, Anti-Gewalt-Trainings und Männerberatung.'
    actions:
      - label: Deutschland
        type: button
        link: 'https://maennerberatungsnetz.de/'
      - label: Österreich
        type: button
        link: 'https://www.oesterreich.gv.at/de/themen/notfaelle_unfaelle_und_kriminalitaet/hilfe-bei-gewalt/Seite.290202'
    _template: cta
---

